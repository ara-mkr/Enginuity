...
- [x] **FIXED — Remote Google Fonts in `index.html`.** DM Sans + JetBrains Mono used to load from `fonts.googleapis.com`. Self-hosted now via `@fontsource-variable/dm-sans` and `@fontsource/jetbrains-mono`, imported in `src/index.css`; the `index.html` preconnect/stylesheet tags are gone and no request to Google Fonts happens at all, so the packaged Electron app gets correct typography offline.
